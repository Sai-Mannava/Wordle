    ...
    // START YOUR CODE HERE
      keyboardController.didSelectString = { [weak self] selectedString in
          ...
          if selectedString == DELETE_KEY {
              ...
              // Call the enter method of boardController and pass in the selected string
              self?.boardController.enter(selectedString)
          }
      }

      if let chatImage = UIImage(named: "Chat") {
          let chatImageView = UIImageView(image: chatImage)
